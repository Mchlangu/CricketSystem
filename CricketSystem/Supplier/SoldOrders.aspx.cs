...
{
    public partial class SoldOrders : System.Web.UI.Page
    {
        ...
        {
            string strQuery = "";
            string username = GetUsername(Convert.ToInt32(lblUsername.Text));

            strQuery = "select a.Order_id, a.Username,a.Date,a.Time,a.Status from order_table a,product_table b  WHERE a.Product_id = b.Product_id AND b.Username = '" + username + "' group by a.Order_id, a.Username,a.Date,a.Time,a.Status order by a.Order_id desc";
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            strQuery = "select Order_id, Username,Date,Time,Status from order_table where OrderBy = 'Admin' And Username like '%" + txtSearch.Text + "%' order by Order_id desc";
        ...
        public void updateStatus(int OrdersNo)
        {
            if (ddlStatus.SelectedIndex > 0)
            {
                using (CricketSystemEntities ctx = new CricketSystemEntities())
                {
                    var p = (from y in ctx.order_table
                             where y.Order_no.Equals(OrdersNo)
                             select y)
                             .ToList();
                    foreach (var x in p)
                    {
